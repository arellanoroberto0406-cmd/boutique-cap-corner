    ...
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          {/* ACCIONES (Search al lado del corazón en desktop) */}
          <div className="flex items-center gap-2 ml-auto">
            {/* Mobile: botón de búsqueda con barra deslizante */}
            ...
            {/* Desktop: Search fijo justo antes del corazón */}
            <div className="hidden lg:block w-[420px] max-w-[48vw]">
              <SearchBar />
            </div>
      ...
      <Dialog open={isBrandsOpen} onOpenChange={(open) => (open ? openBrandsMenu() : closeBrandsMenu())}>
        <DialogContent className="max-w-full h-full w-full p-0 bg-black border-0 [&>button]:!hidden">
          ...
          <DialogDescription className="sr-only">Explora las marcas disponibles en nuestra tienda</DialogDescription>

          <div className="relative h-full w-full overflow-y-auto bg-black" style={{ touchAction: "pan-y" }}>
            {/* Logo centro para volver al inicio */}
            ...
            {imagesLoaded && (
              <div
                className="flex flex-col items-center gap-2 mb-6 max-w-md mx-auto px-4 overflow-y-auto animate-fade-in"
                    ...
                    {category.title === "MARCAS" ? (
                      <details className="w-full group [&>summary]:outline-none">
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-3 p-4 bg-black/50 rounded-md border border-white/10">
                          {/* grid de marcas */}
                          <div
                            onClick={() => { navigate("/bass-pro-shops"); closeBrandsMenu(); }}
                      ...
                      <details className="w-full group [&>summary]:outline-none">
                        ...
                        <div className="mt-2 flex flex-col gap-1 p-2 bg-black/50 rounded-md border border-white/10">
                          {category.items.map((item) => (
                            <button
                              key={item.name}
                              onClick={() => { navigate(item.path); closeBrandsMenu(); }}
